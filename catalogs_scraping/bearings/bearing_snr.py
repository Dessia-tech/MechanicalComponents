...
doc = minidom.parse('../../../SeaFiles/Dossiers_Clients/SNR/POC_Roulement/Doc_SNR/Catalogue_Industrie_E-Shop/Catalogue_Industrie_Standards_PRD.xml')
...
compt = 0
dict_rlts = {'RadialRollerBearing1':{},'RadialRollerBearing2':{},
             'RadialRollerBearing3':{},'RadialRollerBearing4':{},
             'RadialBallBearing1':{}, 'RadialBallBearing2':{},'TaperedRollerBearing1':{}}
...
for element in doc.getElementsByTagName('ICSInstance'):
    ...
    if 'Capacité charge dynamique (Cr)' in dict_temp.keys():
        ...
        dict_temp['Cr'] = float(dict_temp['Cr'])
    ...
    if 'Capacité charge statique (C0r)' in dict_temp.keys():
        ...
        dict_temp['C0r'] = float(dict_temp['C0r'])
    ...
    if 'Charge limite à la fatigue (Cu)' in dict_temp.keys():
        ...
        dict_temp['Cu'] = float(dict_temp['Cu'])
    ...
    if 'typ' in dict_temp.keys():
        if 'Roulements à billes à contact oblique à 2 rangées' in dict_temp['typ']:
            del dict_temp['typ']
            dict_rlts['RadialBallBearing2'][ICSId] = dict_temp
            compt += 1
    if 'typ' in dict_temp.keys():
    ...
    if 'typ' in dict_temp.keys():
        if 'Roulements à rouleaux coniques à 1 rangée' in dict_temp['typ']:
            del dict_temp['typ']
            dict_rlts['TaperedRollerBearing1'][ICSId] = dict_temp
            compt += 1
...
dict_rlts_pandas.to_csv('rlts_SNR.csv',index=False)

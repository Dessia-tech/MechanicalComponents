...
input_values = {workflow_assembly_bearing.index(blockA.inputs[0]): [[[[0.1595, 0, 0], [0, -14000, 0], [0, 0, 0]]]],
                wf.index(blockA.inputs[1]): [157.07],
                wf.index(blockA.inputs[2]): [3600000],
                wf.index(blockA.inputs[3]): [0.035, 0.035],
                wf.index(blockA.inputs[4]): [0.072, 0.072],
                wf.index(blockA.inputs[5]): [0, 0.3], 
                wf.index(blockA.inputs[6]): [0.03, 0.03],
                wf.index(blockA.inputs[7]): [bearings.SelectionLinkage([bearings.Linkage(ball_joint=True), bearings.Linkage(cylindric_joint=True)]),
                                                    bearings.SelectionLinkage([bearings.Linkage(ball_joint=True), bearings.Linkage(cylindric_joint=True)])],
                wf.index(blockA.inputs[8]): [bearings.CombinationMounting([bearings.Mounting(), bearings.Mounting(left=True)])],
                wf.index(blockA.inputs[9]): [[1], [1]],

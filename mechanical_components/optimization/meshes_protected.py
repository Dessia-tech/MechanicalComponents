...
class ContinuousMeshesAssemblyOptimizer:
    ...
    def __init__(self, Z, center_distances, connections, rigid_links, transverse_pressure_angle,
        ...
        # Initailization
        self.solutions=[]
        axial_contact_ratio_gear={}
        total_contact_ratio_min_gear={}
        transverse_contact_ratio_min_gear={}
        for gear in rack_choice.keys():
           
            axial_contact_ratio_gear[gear]=rack_list[rack_choice[gear]].axial_contact_ratio
            total_contact_ratio_min_gear[gear]=rack_list[rack_choice[gear]].total_contact_ratio_min
            transverse_contact_ratio_min_gear[gear]=rack_list[rack_choice[gear]].transverse_contact_ratio_min
            ...
            for (eng1,eng2) in list_connections:
                ...
                if num_cd not in list_analyze_cd.keys():
                    ...
                    if num_line not in list_analyze_line_gear:
                        ...
                        if interval_tpa[0]!=interval_tpa[1]:
                            dict_unknown['transverse_pressure_angle'].append(num_mesh)
                self.axial_contact_ratio[num_mesh]=axial_contact_ratio_gear[eng1]
                self.total_contact_ratio_min[num_mesh]=total_contact_ratio_min_gear[eng1]
                self.transverse_contact_ratio_min[num_mesh]=transverse_contact_ratio_min_gear[eng1]
                num_mesh+=1
...
#            print('optimizer_data {} dict_cd {} num_cd {}'.format(optimizer_data, dict_cd, num_cd))
    ...
    def Fineq(self,X,constraint_root_diameter,CA_min):
        ...
        for num_mesh,mesh_assembly_iter in enumerate(self.mesh_assembly.mesh_combinations):
            ...
            transverse_contact_ratio_min=1
            ...
            for num_mesh,(engr1,engr2) in enumerate(mesh_assembly_iter.connections):
                ...
                if Z2<0:
                    ...
                    if constraint_root_diameter:
                    
                        if constraint_root_diameter[num_mesh]:
                            
                            dia1=mesh_assembly_iter.meshes_dico[engr1].root_diameter
                            dia2=mesh_assembly_iter.meshes_dico[engr2].root_diameter
                            
                            ineq.append((cd- 0.5*(de1-dia2)-Ca_min))
                            ineq.append((cd- 0.5*(-de2+dia1)-Ca_min))
        ...
        return ineq
    def Fineq_without_constraint_ratio_contact(self,X,constraint_root_diameter,CA_min):
    ...
    def Objective(self,X,constraint_root_diameter,CA_min):
        _ = self.update(X)
        fineq=self.Fineq(X,constraint_root_diameter,CA_min)
        obj=0
    ...
    def Objective_helix_angle(self,helix_angle):
        ...
        return obj
    def Optimize(self, verbose = False,constraint_ratio_contact=True,constraint_root_diameter=None,CA_min=None):
        """ Optimizer function
...
#                                break
    ...
    def Optimize(self,nb_sol=1, list_sol=None, verbose=False,constraint_ratio_contact=True):
    ...
    #                print('dw: ', liste_plex[i]['dw'])
            ...
            for plex in liste_plex:
                try:
                    ...
                    continue
                try:
                    ga.Optimize(verbose,constraint_ratio_contact,self.constraint_root_diameter,self.CA_min)
                except ValueError:

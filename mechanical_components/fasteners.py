...
class HexagonNutEnhanced(DessiaObject):
    _standalone_in_db = True
...
class HexagonScrewEnhanced(DessiaObject):
    _standalone_in_db = True
...
class FlatWasherEnhanced(DessiaObject):
    _standalone_in_db = True
...
class BoltEnhanced(DessiaObject):
    def __init__(self, screw:HexagonScrewEnhanced, nut:HexagonNutEnhanced,
                 nut_position:float, washer:FlatWasherEnhanced=None, name:str=''):
        self.screw = screw
        self.nut = nut
        self.nut_position = nut_position
        self.washer = washer
    ...
    @classmethod
    def dict_to_object(cls, dict_):
        screw = HexagonScrewEnhanced.dict_to_object(dict_['screw'])
        nut = HexagonNutEnhanced.dict_to_object(dict_['nut'])
        if "washer" in dict_:
            if dict_['washer'] is not None:
                washer = FlatWasherEnhanced.dict_to_object(dict_['washer'])
...
class ScrewAssemblyEnhanced(DessiaObject):
    def __init__(self, screws:List[HexagonScrewEnhanced], positions:List[float],
                 axis:int, name:str=''):
        self.screws = screws
        self.positions = positions
        self.axis = axis
    ...
    @classmethod
    def dict_to_object(cls, dict_):
        screws = [HexagonScrewEnhanced.dict_to_object(s) for s in dict_['screws']]
        return cls(screws=screws, positions=dict_['positions'],
                   axis=dict_['axis'], name=dict_['name'])

class BoltAssemblyEnhanced(DessiaObject):
    def __init__(self, bolts:List[BoltEnhanced], positions:List[float],
                 axis:int, name:str=''):
        self.bolts = bolts
        self.positions = positions
        self.axis = axis
    ...
    @classmethod
    def dict_to_object(cls, dict_):
        bolts = [BoltEnhanced.dict_to_object(s) for s in dict_['bolts']]
        return cls(bolts=bolts, positions=dict_['positions'],
                   axis=dict_['axis'], name=dict_['name'])

iso_nuts = [HexagonNutEnhanced(0.0016, 0.0032, 0.0013)]
iso_hexscrews = [HexagonScrewEnhanced(0.008, 0.025, 0.008, 0.005, 0.012)]
